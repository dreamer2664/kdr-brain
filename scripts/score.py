"""Score the shipped binary (retrieval + composer) on the question sets in tests/.
   ...
   --strict : exit 1 unless every block of 10 scores >= 9 (CI gate)
...
def ensure_server():
    ...
    SERVER = subprocess.Popen([BIN, BRAIN, "--chat", MODEL, "serve", str(PORT)], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
...
if __name__ == "__main__":
    ...
    worst = 10; grand_ok = grand_n = 0; t_all = 0
    for path in paths:
        ...
        for name, tests in load_tests(path):
            ...
            for q, acc in tests:
                ...
                elif not g: fails.append((q, reply[:200], j["retrieval"]["confidence"]))
            score10 = round(10 * ok_n / len(tests), 1)
            worst = min(worst, score10); grand_ok += ok_n; grand_n += len(tests)
            print(f"  {score10:>4}/10  {name}  ({ok_n}/{len(tests)})")
            for q, r, c in fails: print(f"        FAIL [{c:.2f}] {q}\n             -> {r}")
    print(f"\nTOTAL {grand_ok}/{grand_n} ({100*grand_ok/grand_n:.0f}%)   worst block {worst}/10   avg {t_all/grand_n/1000:.1f}s per reply")
    if strict and worst < 9: print("FAIL: a block scored below 9/10"); sys.exit(1)
