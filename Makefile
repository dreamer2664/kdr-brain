...
release/kdr-brain: src/brain.c src/main.c src/chat.c src/webui.c src/brain.h src/chat.h src/unicode_tables.h $(LLAMA_LIBS)
	mkdir -p release
	...
	$(CXX) -O3 -fopenmp -o $@ /tmp/kdr_brain.o /tmp/kdr_main.o /tmp/kdr_chat.o /tmp/kdr_webui.o $(LLAMA_LIBS) -static -fopenmp -lm -lpthread
	strip $@
	@ls -la $@

# retrieval-only build (no llama.cpp needed, no chat)
lite: src/brain.c src/main.c src/webui.c
	mkdir -p release
	$(CC) $(CFLAGS) -DKDR_NO_CHAT -o release/kdr-brain-lite src/brain.c src/main.c src/webui.c $(LDFLAGS) && strip release/kdr-brain-lite
